...
let selectedInstrument = 'Guitarra/Violão (6 Cordas)';
let numberOfCordas = instrumentTuningPresets[selectedInstrument].length;
...
const app = {
  ...
  setupFretboard() {
    fretboard.innerHTML = '';
    root.style.setProperty('--number-of-Cordas', numberOfCordas);
    // Add Cordas to fretboard
    for(let i = 0; i < numberOfCordas; i++) {
...
const handlers = {
  ...
  setSelectedInstrument(event) {
    ...
    numberOfCordas = instrumentTuningPresets[selectedInstrument].length;
    app.setupFretboard();
  },
